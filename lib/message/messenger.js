...
Messenger.prototype.deliver = function( envelope, rinfo ) {
    ...
    for( mesg of envelope.payload ) {
        ...
        if( !envelope.local && mesg.ack ) {
          this.sendGlobal( [new Messages.ACK( mesg.ack )], envelope.from );
        }

        // If acknowledge packet recieved, append uid to event
        if ( mesg.type === 'ack' ) {
          this.emit(mesg.type + mesg.uid, mesg, envelope.from);
        } else {
          // Emit the message
 ...
 */
Messenger.prototype.sendLocal = function( payload, recipient ) {
    ...
    // If we have a revipient id then this is a packet destined for one person
    if( recipient.id !== undefined ) {
	    islocal = false;
        thierid = recipient.id;
    }
