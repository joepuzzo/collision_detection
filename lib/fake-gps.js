 ...
 */
function FakeGPS( plan, speed, params ) { 
...
FakeGPS.prototype.advertise = function( config ) {
    ...
    var interval = setInterval(function () {
        my.emit( "location", my.getLocation() );
    }, config.location_interval || 100 );

    // Add this interval to map 
    this.intervals.set( "location", interval ); 

}

/*----------------AdvertismentShutdown------------------*/
FakeGPS.prototype.cleanup = function() {
...
FakeGPS.prototype.pause = function( ) { 
    ...
    var child = spawn('coresendmsg' ,['node', 'number='+this.params.id, 'icon=/home/vagrant/tmp/drone-icon-red.png', '-a' ,'172.16.1.254']);
...
FakeGPS.prototype.unpause = function( ) { 
    ...
    var child = spawn('coresendmsg' ,['node', 'number='+this.params.id, 'icon=/home/vagrant/tmp/drone-icon.gif', '-a' ,'172.16.1.254']);
    child.on('error', function(err) {
    	//console.log("ERROR:",err);
    });
