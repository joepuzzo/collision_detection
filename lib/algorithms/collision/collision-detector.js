...
CollisionDetector.prototype.altitudeCheck = function( seg1, seg2, collision ) { 
    // calculate the difference in the z's
    var zdiff = Math.abs( seg1.start.alt - seg2.start.alt )

    // Add some data to the collision object
    // TODO this should be smarter for seinor project
    //      I dont like this data ( put more relevent data eventually )
    collision.isect1.alt = seg1.start.alt;
    collision.isect2.alt = seg2.start.alt;
...
CollisionDetector.prototype.positionCheck = function( seg1, seg2, collision ) { 
    ...
    var line1 = new LineUtils.Line( seg1.start.lat, seg1.start.lon, seg1.end.lat, seg1.end.lon ),
        line2 = new LineUtils.Line( seg2.start.lat, seg2.start.lon, seg2.end.lat, seg2.end.lon );
    ...
    if( result.onLine1 && result.onLine2 ) {
        ...
        collision.isect1.lat =  result.x; 
        collision.isect1.lon =  result.y;  
        collision.isect2.lat =  result.x; 
        collision.isect2.lon =  result.y; 
