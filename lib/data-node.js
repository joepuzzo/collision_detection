...
function DataNode( params, callback ) { 
    ...
    // Setup core messages 
    var child;
    // Register to send gps info to core
    this.gps.on("location", function( loc ){ 
        child = spawn('coresendmsg' ,['node', 'number='+params.id, 'xpos='+Math.floor( loc.lat ), 'ypos='+Math.floor( loc.lon ), '-a' ,'172.16.1.254']);
        // Error will be thrown when core is not present
        child.on('error', function(err) {
          //console.log("ERROR:",err);
        });
    });

    var child = spawn('coresendmsg' ,['node', 'number='+params.id, 'icon=/home/vagrant/tmp/drone-icon.gif', '-a' ,'172.16.1.254']);
    child.on('error', function(err) {
    	//console.log("ERROR:",err);
    });
...
/*-----------AdvertismentCleanupAndServerShutdown--------*/
