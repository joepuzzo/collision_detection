...
CollisionSystem.prototype.handleScheduleReply = function( message, from ) {
    ...
    if(this.coordLinks.get(from.id)!==undefined && !this.coordLinks.get(from.id).gotSchedule){
    ...
    for(var trajSeg of schedule){
	    ...
	    console.log("MY SEGPERMS ARE: ",this.segPermissions);
            this.segPermissions.get(Trajectory.stringify(mySeg)).push(from.id);
            if(Trajectory.stringify(this.gps.curTrajectory)==Trajectory.stringify(mySeg)){
                this.gps.pause();
            }
        }else if(act==2){
