...
CollisionSystem.prototype.handleDeadlockProbe = function( message, from ) {
	...
	if(segPermissions.get(JSON.stringify(traj)).length>0){ //send message to all people you are waiting on
	    var waitIds = segPermissions.get(JSON.stringify(traj)); for(var id of waitIds){
