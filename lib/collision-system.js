...
function CollisionSystem( messenger, gps, id) { 
    ...
    //TODO did you mean to use in here ? traj is an integer!! 
    //TODO change this.plan to be this.plan.trajectories!!!
    for(var traj in this.plan){
	    segPermissions.set(traj,[]);
    }
...
CollisionSystem.prototype.handleHIA = function( message, from ) { 
	...
	console.log("GOT HIA IN COLSYSTEM"); 
...
CollisionSystem.prototype.handleRollAssignment = function( message, from ) {
    ...
    console.log("FUCK YEAH!! GOT THE ROLLASSIGNMENT");
    console.log(this.id);   
    if(this.coordLinks.get(from.id)==undefined){
    	this.coordLinks.set(from.id,new CoordLink(fom,message.roll));
    }
...
CollisionSystem.prototype.handleScheduleReply = function( message, from ) {
    ...
    //TODO if myPlan is a plan object then you are itterating over nothing here! you need to put myPlan.trajectories
    //TODO If its not a plan object then nevermind
...
CollisionSystem.prototype.cleanupCoordLink = function(linkpartnerid){
    ...
    //remove coordlink
    // TODO YOU USED in!! you are itterating over a bunch of integers! 
    for(var seg in this.segPermissions){
	    if(this.segPermissions[seg].contains(linkpartnerid)){
	        //forget about the fact you were waiting on them
	        var ind = this.segPermissions[seg].indexOf(linkpartnerid);
	        this.segPermissions[seg].splice(index,1);
  	    }
    }
