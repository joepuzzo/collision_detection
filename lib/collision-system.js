...
CollisionSystem.prototype.handlePlanResponse = function( message, from ) {
	...
	*/
	...
	if(collision){
	    ...
	    if(collision.trajectory1.timeOfCollision<collision.trajectory2.timeOfCollision){
...
CollisionSystem.prototype.removeFromSegPerm =function(granter){
     for(var seg of this.segPermissions){
        var trajSeg   = seg[0];
        var waitingOn = seg[1];
        if( waitingOn.contains(linkpartnerid)){
            //forget about the fact you were waiting on them
            var ind = waitingOn.indexOf(linkpartnerid);
            waitingOn.splice(ind,1);
            if(JSON.stringify(trajSeg)==JSON.stringify(this.gps.curTrajectory)){
                //we are removing permissions for current seg
                //if zero, WE ARE FREE to go
                if(waitingOn.length==0){
                    console.log("FUCK YEAH!!!! WE ARE FREE!!!!");
                    this.gps.unpause();
                }else{
...
CollisionSystem.prototype.handleScheduleReply = function( message, from ) {
	...
	if(act==1){
	    ...
	    this.gps.when(collision.isect2,collision.trajectory2.trajectory,
	        this.collisionDetetector.dcollide,function(from){
		var permission = new GrantPermission();
		this.messenger.sendLocal([permission],from);	
	    });
	}
    }
