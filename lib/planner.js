...
Plan.prototype.getWaitTime = function() { 
    ...
    return { time: wait_time, stops: stops }; 
}
...
/**
 * This function will return this plan as a visplot string
 */
Plan.prototype.toVisplot = function( params ) { 
    // Itterate over all the trajectories and call to bon string
    var plotString = [];
    var curString  = [];
    for( t of this.trajectories ) {
         // Always push on the lat and lon
         curString = [t.lat, t.lon];
         // If the caller specified time then write out the time
         if( params.alt  ) curString.push( t.alt );
         if( params.time ) curString.push( t.time );
         plotString.push( curString.join(" ") + "\n" ); 
    } 
    return plotString.join("").trim();;
}
